...
var resizePizzas = function(size) { 
  ...
  function changePizzaSizes(size) {
	...
	var rpizzalength = rpizza.length;
	var i;
    for (i = 0; i < rpizzalength; i++) {
...
// Moves the sliding background pizzas based on scroll position
  ...
  var phase;
  var finalpos =[];
  var widthActualHalf = (window.innerHeight)/2;
...
function updatePositions() {
  ...
  scrsin = scrolltop / 1250;   
  var i;
  window.performance.mark("mark_start_frame");
  for (i = 0; i < itemslength; i++) {
    phase = (Math.sin(scrsin + modPos[i])) * 100;
	finalpos[i] = items[i].basicLeft + phase - (widthActualHalf + 220);
    items[i].style.transform = "translateX(" + finalpos[i] + "px)";
